...
import { loginSchema, profileSchema, registerSchema, } from "../validators/userValidator.js";
import { User } from "../models/userModel.js";
import constants from "../constants/constants.js";
import AppError from "../utils/AppError.js";
const validationError = (error) => new AppError(`Validation Error: ${error.errors.join(", ")}`, constants.BAD_REQUEST);
export const registerUser = async (req, res, next) => {
    try {
        const { firstName, lastName, email, password } = await registerSchema.validate(req.body, { abortEarly: false });
        const userAvailable = await User.findOne({ email });
        if (userAvailable) {
            return next(new AppError("User already registered with this email", constants.BAD_REQUEST));
        }
...
export const updateUser = async (req, res, next) => {
    try {
        ...
        const user = await User.findByIdAndUpdate(req.user.userId, { phone, address, avatar }, { new: true, runValidators: true });
        if (!user) {
